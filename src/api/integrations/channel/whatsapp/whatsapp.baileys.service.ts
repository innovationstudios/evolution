...
export class BaileysStartupService extends ChannelStartupService {
        ...
        if (key.remoteJid !== 'status@broadcast') {
          ...
          const findMessage = await this.prismaRepository.message.findFirst({
            where: {
              instanceId: this.instanceId,
              key: {
                path: '$.id', // ✅ JSON path correto para MySQL
                equals: key.id,
              },
            },
          });
  ...
  private async updateMessagesReadedByTimestamp(remoteJid: string, timestamp?: number): Promise<number> {
    ...
    const result = await this.prismaRepository.message.updateMany({
      where: {
        AND: [
          { key: { path: '$.remoteJid', equals: remoteJid } }, // ✅ string, não array
          { key: { path: '$.fromMe', equals: false } }, // ✅ string, não array
          { messageTimestamp: { lte: timestamp } },
          { OR: [{ status: null }, { status: status[3] }] },
        ],
      },
      data: { status: status[4] }, // atualizado para "READ" ou status correspondente
    });
  ...
  private async updateChatUnreadMessages(remoteJid: string): Promise<number> {
    // Busca o chat e conta mensagens não lidas em paralelo
      ...
      this.prismaRepository.message.count({
        where: {
          AND: [
            ...
            { status: { equals: status[3] } }, // DELIVERY_ACK, por exemplo
          ],
        },
      }),
    ]);

    // Atualiza o campo unreadMessages apenas se houver diferença
